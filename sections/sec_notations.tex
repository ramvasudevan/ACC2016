...
\end{align*}
Let $s\mapsto \mho(s)$ represent the function that relates each sequence of mode-transitions to all possible instantiations of parametric uncertainties.
\par
For convenience, with a slight abuse of notations, let the execution of $\mathcal H$ with initial condition and $x(0)=x_0$ and terminal time $T$ be denoted as $\mathcal H(x_0,j,T)$
\begin{align}
    \mathcal H\colon \mathcal D\times \R\rightarrow \mathcal C_{ab}(\R;\cup_{j\in \mathcal J} M_j)\times \{\cup_{s\in S(x_0)} \mho(s)\}.
\end{align}
That is, $\mathcal H$ outputs the temporal evolution of states of $\mathcal H$, and the discrete sequence of disturbances encountered.
\par
Then, $X_{(0,j)}$ is defined as the following
\begin{align}
\begin{aligned}
     X_{(0,j)}:=\{y\in \mathrm M_j\mid (x,\rho)=\mathcal H(y,j,T),\\\,x(T)\in X_T,\forall \rho\in \mho(x_0) \}
\end{aligned}
\end{align}
