...
\begin{defn}[Inspired by \cite{Burden2015}]\label{def:system}
  ...
  \begin{itemize}
    ...
    \item $\mathcal G=\{G_e\}_{e \in {\cal E}}$ is the set of guards where each $G_(j,j') \in \partial D_j$ is a guard in domain $j \in J$ that defines a transition to mode $j' \in {\cal J}$;
