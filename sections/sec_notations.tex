...
\begin{defn}[Inspired by \cite{Burden2015}]\label{def:system}
  ...
  \begin{itemize}
    ...
    \item $\mathcal F=\{\tilde f_j\}_{j\in \mathcal J}$ where $\tilde{f}_j: D_j \times \Theta_j \to D_j$ is the vector field defining the dynamics on $D_j$;
    \item $\mathcal G=\{G_e\}_{e \in {\cal E}}$ is the set of guards where each $G_{(j,j')} \subset \partial D_j$ is a guard in domain $j \in J$ that defines a transition to mode $j' \in {\cal J}$;
